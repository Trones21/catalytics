...
dirForThisTest="_temp_/jq_testing/"
...
actualDCVar=$(jq '.catalytics.overall.docCount' $fileUri)
if [[ "$actualDCVar" == "null" ]]; then
    echo "Check Failed: .catalytics.overall.docCountSelf is not present in _category_.json"
else 
...
actualdt=$(jq '.catalytics.overall.runDateTime' $fileUri)
...
## === Check 3:  === ##
echo ""
### ToDo correct amount of items in an array
